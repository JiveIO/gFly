...
func (d *insertData) ToSql() (sqlOut string, args []interface{}, err error) {
	if d.Into == "" {
	...
	if d.StatementKeyword == "" {
		sqlStr.WriteString("INSERT ")
	} else {
		sqlStr.WriteString(d.StatementKeyword)
		sqlStr.WriteString(" ")
	}
	...
	sqlStr.WriteString(d.Into)
	sqlStr.WriteString(" ")

	if len(d.Columns) > 0 {
		sqlStr.WriteString("(")
		sqlStr.WriteString(strings.Join(d.Columns, ","))
		sqlStr.WriteString(") ")
	}

	if d.Select != nil {
		args, err = d.appendSelectToSQL(sqlStr, args)
	} else {
		args, err = d.appendValuesToSQL(sqlStr, args)
	}
	if err != nil {
		return
	}
...
func (b InsertBuilder) Prefix(sqlStr string, args ...interface{}) InsertBuilder {
...
func (b InsertBuilder) Suffix(sqlStr string, args ...interface{}) InsertBuilder {
