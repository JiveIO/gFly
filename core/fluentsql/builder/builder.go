...
var (
	EmptyBuilder      = Builder{data.NewMap()}
	emptyBuilderValue = reflect.ValueOf(EmptyBuilder)
)

func getBuilderMap(builder interface{}) data.Map {
...
func listToSlice(list data.List, arrayType reflect.Type) reflect.Value {
	size := list.Size()
...
func GetMap(builder interface{}) map[string]interface{} {
	...
	m.ForEach(func(name string, val data.Any) {
		list, isList := val.(data.List)
		if isList {
			arrayType := anyArrayType
