...
func (d *selectData) toSqlRaw() (sqlOut string, args []interface{}, err error) {
	if len(d.Columns) == 0 {
	...
	if len(d.Options) > 0 {
		sqlStr.WriteString(strings.Join(d.Options, " "))
		sqlStr.WriteString(" ")
	}

	if len(d.Columns) > 0 {
		args, err = appendToSql(d.Columns, sqlStr, ", ", args)
		if err != nil {
			return
		}
	}
	...
	if len(d.Joins) > 0 {
		sqlStr.WriteString(" ")
		args, err = appendToSql(d.Joins, sqlStr, " ", args)
		if err != nil {
			return
		}
	}
	...
	if len(d.GroupBys) > 0 {
		...
		sqlStr.WriteString(strings.Join(d.GroupBys, ", "))
	}

	if len(d.HavingParts) > 0 {
		sqlStr.WriteString(" HAVING ")
		args, err = appendToSql(d.HavingParts, sqlStr, " AND ", args)
		if err != nil {
			return
		}
	}
	...
	if len(d.OrderByParts) > 0 {
		...
		args, err = appendToSql(d.OrderByParts, sqlStr, ", ", args)
		if err != nil {
			return
		}
	}
...
func (b SelectBuilder) Prefix(sqlStr string, args ...interface{}) SelectBuilder {
...
func (b SelectBuilder) Suffix(sqlStr string, args ...interface{}) SelectBuilder {
