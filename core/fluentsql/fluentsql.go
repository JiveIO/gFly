// Package fluentsql provides a fluent SQL generator.
//
...
func DebugSqlizer(s Sqlizer) string {
	...
	i := 0
	for {
		p := strings.Index(sqlStr, placeholder)
		if p == -1 {
			break
		}
		if len(sqlStr[p:]) > 1 && sqlStr[p:p+2] == "??" { // escape ?? => ?
			buf.WriteString(sqlStr[:p])
			buf.WriteString("?")
			if len(sqlStr[p:]) == 1 {
				break
			}
			...
			// advance our sqlStr string "cursor" beyond the arg we placed
			sqlStr = sqlStr[p+1:]
			i++
		}
	}
	...
	// "append" any remaning sqlStr that won't need interpolating
