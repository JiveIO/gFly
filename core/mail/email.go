// Package mail is designed to provide an "email interface for humans."
...
func writeMessage(buff io.Writer, msg []byte, isMultipart bool, mediaType string, w *multipart.Writer) error {
	if isMultipart {
		header := textproto.MIMEHeader{
...
func (at *Attachment) setDefaultHeaders() {
	...
	if at.Header.Get("Content-Disposition") == "" {
	...
	if at.Header.Get("Content-Transfer-Encoding") == "" {
