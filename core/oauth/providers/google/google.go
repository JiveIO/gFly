...
// Provider is the implementation of `oauth.Provider` for accessing Google.
...
func (p *Provider) FetchUser(session oauth.Session) (oauth.User, error) {
	...
	// Extract the user data we got from Google into our oauth.User.
	user.Name = u.Name
...
func (p *Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	...
	ts := p.config.TokenSource(oauth.ContextForClient(p.Client()), token)
