...
var (
	AuthURL    = utils.CopyString("https://github.com/login/oauth/authorize")
	TokenURL   = utils.CopyString("https://github.com/login/oauth/access_token")
	ProfileURL = utils.CopyString("https://api.github.com/user")
	EmailURL   = utils.CopyString("https://api.github.com/user/emails")
)
...
// Provider is the implementation of `oauth.Provider` for accessing Github.
...
func (p *Provider) FetchUser(session oauth.Session) (oauth.User, error) {
	...
	req, err := http.NewRequest("GET", p.profileURL, http.NoBody)
	if err != nil {
		return user, err
	}
...
func getPrivateMail(p *Provider, sess *Session) (email string, err error) {
	req, _ := http.NewRequest("GET", p.emailURL, http.NoBody)
	req.Header.Add("Authorization", "Bearer "+sess.AccessToken)
	response, err := p.Client().Do(req)
	if err != nil {
		if response != nil {
